...
# reef parameters
surv_pars.r <- list()
surv_pars.r[[1]] <- 1- c(0.24, 0.13, 0.05, 0.001) # probabilities of surviving

growth_pars.r <- list()
growth_pars.r[[1]] <- list(c(0.197, 0.0132, 0), c(0.126, 0), 0.1158, NULL)

shrink_pars.r <- list()
shrink_pars.r[[1]] <- list(NULL, 0.08, c(0, 0.105), c(0, 0.06, 0.11))

fec_pars.r <- list()
fec_pars.r[[1]] <- c(0, 0.1, 1, 2)

# orchard parameters
surv_pars.o <- list()
surv_pars.o[[1]] <- 1- c(0.24, 0.13, 0.05, 0.001) # probabilities of surviving

growth_pars.o <- list()
growth_pars.o[[1]] <- list(c(0.197, 0.0132, 0), c(0.126, 0), 0.1158, NULL)

shrink_pars.o <- list()
shrink_pars.o[[1]] <- list(NULL, 0.08, c(0, 0.105), c(0, 0.06, 0.11))

fec_pars.o <- list()
fec_pars.o[[1]] <- c(0, 10, 50, 100)
...
dist_effects.r <- list()
dist_effects.r[[1]] <- list() # list where ith element is the effects of the ith disturbance
dist_effects.r[[1]][[1]] <- c("survival")
...
# first reef subpop
dist_surv.r <- list()
dist_surv.r[[1]] <- surv_pars.r[[1]]*0.1 # survival probabilities in first disturbance year in first reef subpop

dist_Tmat.r <- list()
dist_fec.r <- list()

dist_pars.r[[1]] <- list(dist_surv = dist_surv.r, dist_Tmat = dist_Tmat.r, dist_fec = dist_fec.r) # list of disturbance parameters for the first reef subpop

# disturbance effects for each orchard subpop
dist_effects.o <- list()
dist_effects.o[[1]] <- list() # list where ith element is the effects of the ith disturbance
dist_effects.o[[1]][[1]] <- c("survival")
...
dist_surv.o <- list()
dist_surv.o[[1]] <- surv_pars.o[[1]]*0.1 # survival probabilities in first disturbance year in first reef subpop

dist_Tmat.o <- list()
dist_fec.o <- list()

dist_pars.o[[1]] <- list(dist_surv = dist_surv.o, dist_Tmat = dist_Tmat.o, dist_fec = dist_fec.o) # list of disturbance parameters for the first reef subpop
...
reef1_tot <- apply(sim1$reef_pops[[1]], MARGIN = 2, sum)
orch1_tot <- apply(sim1$orchard_pops[[1]], MARGIN = 2, sum)
...
for(i in 1:years){
  ...
  reef1_Atot[i] <- sum(sim1$reef_pops[[1]][,i]*A_mids)
  
}


plot(x = c(1:years), y = reef1_Atot, type = "l", las = 1, ylim = c(0.9*reef_areas[1], max(reef1_Atot)), xlab = "year", ylab = "Area covered", lwd = 2) # ylim = c(0, max(reef1_Atot))
...
surv_pars.r <- list()
surv_pars.r[[1]] <- 1- c(0.24, 0.13, 0.05, 0.001) # probabilities of surviving
surv_pars.r[[2]] <- surv_pars.r[[1]] # 1-3 are for recruits originating in the first lab treatment
surv_pars.r[[3]] <- surv_pars.r[[1]]
surv_pars.r[[4]] <- surv_pars.r[[1]] # 4-6 are same three reef subpops but for recruits originating in the second lab treatment
surv_pars.r[[5]] <- surv_pars.r[[1]]
surv_pars.r[[6]] <- surv_pars.r[[1]]

growth_pars.r <- list()
growth_pars.r[[1]] <- list(c(0.197, 0.0132, 0), c(0.126, 0), 0.1158, NULL)
growth_pars.r[[2]] <- growth_pars.r[[1]]
growth_pars.r[[3]] <- growth_pars.r[[1]]
growth_pars.r[[4]] <- growth_pars.r[[1]]
growth_pars.r[[5]] <- growth_pars.r[[1]]
growth_pars.r[[6]] <- growth_pars.r[[1]]

shrink_pars.r <- list()
shrink_pars.r[[1]] <- list(NULL, 0.08, c(0, 0.105), c(0, 0.06, 0.11))
shrink_pars.r[[2]] <- shrink_pars.r[[1]]
shrink_pars.r[[3]] <- shrink_pars.r[[1]]
shrink_pars.r[[4]] <- shrink_pars.r[[1]]
shrink_pars.r[[5]] <- shrink_pars.r[[1]]
shrink_pars.r[[6]] <- shrink_pars.r[[1]]

fec_pars.r <- list()
fec_pars.r[[1]] <- c(0, 0.1, 1, 2)
fec_pars.r[[2]] <- fec_pars.r[[1]]
fec_pars.r[[3]] <- fec_pars.r[[1]]
fec_pars.r[[4]] <- fec_pars.r[[1]]
fec_pars.r[[5]] <- fec_pars.r[[1]]
fec_pars.r[[6]] <- fec_pars.r[[1]]

# orchard parameters
surv_pars.o <- list()
surv_pars.o[[1]] <- 1- c(0.24, 0.13, 0.05, 0.001) # probabilities of surviving
surv_pars.o[[2]] <- surv_pars.o[[1]]
surv_pars.o[[3]] <- surv_pars.o[[1]]
surv_pars.o[[4]] <- surv_pars.o[[1]]

growth_pars.o <- list()
growth_pars.o[[1]] <- list(c(0.197, 0.0132, 0), c(0.126, 0), 0.1158, NULL)
growth_pars.o[[2]] <- growth_pars.o[[1]]
growth_pars.o[[3]] <- growth_pars.o[[1]]
growth_pars.o[[4]] <- growth_pars.o[[1]]

shrink_pars.o <- list()
shrink_pars.o[[1]] <- list(NULL, 0.08, c(0, 0.105), c(0, 0.06, 0.11))
shrink_pars.o[[2]] <- shrink_pars.o[[1]]
shrink_pars.o[[3]] <- shrink_pars.o[[1]]
shrink_pars.o[[4]] <- shrink_pars.o[[1]]

fec_pars.o <- list()
fec_pars.o[[1]] <- c(0, 10, 50, 100)
fec_pars.o[[2]] <- fec_pars.o[[1]]
fec_pars.o[[3]] <- fec_pars.o[[1]]
fec_pars.o[[4]] <- fec_pars.o[[1]]
...
reef_out_props[1,] <- c(1/3, 1/3, 1/3) # from first lab treatment to each reef treatment
reef_out_props[2,] <- c(1/3, 1/3, 1/3) # from second lab treatment to each reef treatment
...
sim1 <- rse_mod(years, n, A_mids, surv_pars.r, growth_pars.r, shrink_pars.r, fec_pars.r, surv_pars.o, growth_pars.o, shrink_pars.o, fec_pars.o, lambda, sigma_s, sigma_f, ext_rand, seeds, orchard_treatments, reef_treatments, lab_treatments, lab_pars, rest_pars, N0.r, N0.o, N0.l)
