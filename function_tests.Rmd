...
lab_treatments <- c("1_none") # lab: all need to have prefix of either "0" (immediately outplanted) or "1" (outplanted after 1 year)
...
rest_pars <- list(orchard_yield = orchard_yield, reef_yield = reef_yield, lab_props = lab_props, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, orchard_size = orchard_size, transplant = transplant)
...
fec_pars.r[[1]][[1]] <- c(0, 0, 0.1, 1, 2) # first source
...
#n_retain <- c(0, 0) # max number of new settlers retained in each lab treatment in the lab for a year 
# lab_treatments <- c("0_tile1", "1_tile1") # lab
...
orchard_out_props[1,] <- c(0, 1) # from first lab treatment to each orchard treatment
orchard_out_props[2,] <- c(1, 0) # from second lab treatment to each orchard treatment
