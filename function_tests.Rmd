...
new parameters to add: reef_yield, n_retain, size_props
...
dist_Tmat <- list()
dist_Tmat[[1]] <- G_fun(years = 1, n, list(col1[2:4]*0.9, col2[3:4]*0.9, col3[4]*0.9, NULL), shrink_pars1, frag_pars1)[[1]]
dist_Tmat[[2]] <- G_fun(years = 1, n, growth_pars1, shrink_pars1, frag_pars1)[[1]]
...
reef_prop <- 0.75 # proportion of lab recruits from each lab treatment outplanted to reef (1-proportion outplanted to orchard)
...
rest_pars <- list(orchard_yield = orchard_yield, reef_yield = reef_yield, lab_props = lab_props, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, orchard_size = orchard_size)
...
frag_pars.r[[1]][[1]] <- list(NULL, c(0), c(0, 0), c(0, 0, 0)) # first source
...
n_retain <- c(100, 100) # max number of new settlers retained in each lab treatment in the lab for a year 
size_props <- matrix(NA, nrow = length(lab_treatments), ncol = n) # matrix for fractions of retained recruits in each size class at the end of their year in the lab
size_props[1, ] <- c(1, 0, 0, 0) # first lab treatment
size_props[2, ] <- c(1, 0, 0, 0) # second lab treatment
lab_pars <- list(sett = sett, s = s, n_retain = n_retain, size_props = size_props)
