...
spawn_target <- 1000000 # target number of embryos to collect each year (note: if this is set to zero, there won't be any settlers)
reef_prop <- c(0.75, 0.75) # proportion of lab recruits from each lab treatment outplanted to reef (1-proportion outplanted to orchard)
...
lab_max <- 100 # total number of settlers that the lab can accommodate (need to check with Fundemar -- they said 4000 tiles can fit in lab but 500 per spawning event)
...
orchard_size <- c(30*80) # number of tiles each orchard has space for (80 stars x 30 tiles per star)
