...
# update: make this a matrix to allow for disturbance events in the lab
...
reef_prop <- c(0, 0) # proportion of lab recruits from each lab treatment outplanted to reef (1-proportion outplanted to orchard)
...
check tile calculations

```{r}

# ex.: have 200 tiles in two orchards, 50% were from treatment 1 = 100 tiles from treatment 1 across both orchards, and know that 25% of the tiles from treatment 1 go to orchard 1, so there should be 25 tiles from treatment 1 in orchard 1. Then assume 20% of tiles from treatment 2 go to orchard 1, so there should be 20 tiles from treatment 2 in orchard 1. So there should be a total of 45 tiles in orchard 1
# 25 + 20 + 75 + 80

# then starting from the total number of tiles in orchard 1, want to calculate the number of these tiles that came from each treatment

# 45 = 200*0.5*0.25 + 200*0.5*0.2 --> need to total number of tiles across both orchards
...
# want to calculate the number of tiles from treatment 1 in orchard 1 from the total number of tiles in orchard 1
...
(sim1$reef_out[[1]][[2]] + sim1$reef_out[[1]][[3]])/sim1$tiles_out_tot
...
For calculating approximate area of reef outplanted: have an approximate outplanting density of 4-6 substrates with recruits per m2 (from annual report)
...
number of adults in orchard goes down below max because corals die so it's not possible to keep it 100% full of adults (which seems realistic?), but if you make survival 100% then it does fill up (or even just survival of size classes 1 and 2). 

also the reason the decline is big is because all the tiles are identical so if one dies on one tile, that means one dies on every tile so it's multiplied by the number of tiles

orchard can be full of substrates but not full of adults because some of the substrates don't have adults on them (or they did and then the adults shrunk). Note if you turn off shrinkage, you bounce back up to max capacity even with default survival. 
