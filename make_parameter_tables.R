...
# ----------------------------------------------------------------------------
economic <- data.frame(
  Category = c("Rate", rep("Fixed", 4), rep("Variable", 8)),
  Parameter = c(
...
# ----------------------------------------------------------------------------
# RENDER  (pretty: gt -> HTML + PNG; flextable -> combined Word .docx; + CSV)
# ----------------------------------------------------------------------------
suppressPackageStartupMessages({
  library(gt); library(flextable); library(officer)
})

out_dir <- "tables"
.f <- sub("--file=", "", grep("--file=", commandArgs(FALSE), value = TRUE))
if (length(.f) == 1 && nzchar(.f)) out_dir <- file.path(dirname(.f), "tables")
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

src_note <- "Generated by make_parameter_tables.R; values mirror rse_new_scenario_analyses.rmd."
HEADER_FILL <- "#1f3b57"   # dark slate blue
STRIPE_FILL <- "#eef3f8"

# ---- gt: pretty HTML + PNG, one file per table -----------------------------
make_gt <- function(df, title, subtitle, stub, group_col = NULL) {
  g <- gt(df, groupname_col = group_col, rowname_col = NULL)
  g <- tab_header(g, title = md(paste0("**", title, "**")), subtitle = subtitle)
  g <- tab_source_note(g, source_note = md(paste0("*", src_note, "*")))
  g <- opt_table_font(g, font = "Helvetica")
  if ("Symbol" %in% names(df))
    g <- tab_style(g, cell_text(font = system_fonts("monospace-code"), size = px(11)),
                   cells_body(columns = "Symbol"))
  g <- tab_style(g, cell_text(color = "white", weight = "bold"),
                 cells_column_labels(everything()))
  g <- tab_style(g, cell_fill(color = HEADER_FILL), cells_column_labels(everything()))
  g <- opt_row_striping(g)
  g <- tab_options(g,
    table.font.size = px(12),
    heading.title.font.size = px(15),
    heading.subtitle.font.size = px(11),
    heading.align = "left",
    row.striping.background_color = STRIPE_FILL,
    data_row.padding = px(4),
    table.border.top.color = HEADER_FILL,
    column_labels.border.bottom.color = HEADER_FILL,
    table_body.hlines.color = "#d6dee6",
    source_notes.font.size = px(9))
  gtsave(g, file.path(out_dir, paste0(stub, ".html")))
  png_path <- file.path(out_dir, paste0(stub, ".png"))
  ok <- tryCatch({ gtsave(g, png_path, expand = 8, zoom = 2); TRUE },
                 error = function(e) { message("  (PNG skipped: ", conditionMessage(e), ")"); FALSE })
  write.csv(df, file.path(out_dir, paste0(stub, ".csv")), row.names = FALSE)
  invisible(ok)
}

make_gt(demographic, "Table 1. Demographic parameters.",
        "Components of eqn. 1 and the size-dependent fecundity schedule.",
        "Table1_demographic_parameters")
make_gt(restoration, "Table 2. Restoration parameters.",
        "Restoration decision points, lab/orchard capacities, and disturbance.",
        "Table2_restoration_parameters")
make_gt(economic, "Table 3. Economic parameters (2025 USD).",
        "Base costs; inflated annually via eqn. 4.",
        "Table3_economic_parameters", group_col = "Category")

# ---- flextable: one combined, editable Word .docx --------------------------
style_ft <- function(df, caption) {
  ft <- flextable(df)
  ft <- set_caption(ft, caption)
  ft <- theme_booktabs(ft)
  ft <- bold(ft, part = "header")
  ft <- bg(ft, bg = HEADER_FILL, part = "header")
...
docx_path <- file.path(out_dir, "parameter_tables.docx")
doc <- read_docx()
doc <- body_add_flextable(doc, style_ft(demographic, "Table 1. Demographic parameters."))
doc <- body_add_par(doc, "")
doc <- body_add_flextable(doc, style_ft(restoration, "Table 2. Restoration parameters."))
doc <- body_add_par(doc, "")
doc <- body_add_flextable(doc, style_ft(economic, "Table 3. Economic parameters (2025 USD)."))
doc <- body_add_par(doc, src_note, style = "Normal")
print(doc, target = docx_path)

cat("Parameter tables written to:", normalizePath(out_dir), "\n")
cat("  Table{1,2,3}_*.html  (pretty, open in a browser)\n")
cat("  Table{1,2,3}_*.png   (preview images)\n")
cat("  Table{1,2,3}_*.csv   (data, for Google Sheets / Word import)\n")
cat("  parameter_tables.docx (all three, editable Word tables for the manuscript)\n")
