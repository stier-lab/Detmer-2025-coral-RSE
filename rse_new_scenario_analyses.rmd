...
ylab1 <- expression("Reef cover ("*m^2*")")
#plotlab <- "c) Reef ROI"
plot(x = c(1:yrs_p), y = rep(0, yrs_p), type = "l", col = NA, ylim = c(0, 15), las = 1, xlab = NA, ylab = NA)
mtext(side = 1, xlab1, line = 2.5, outer = T, cex = lab_cx)
mtext(side = 2, ylab1, line = 3.5, cex = lab_cx)
#mtext(side = 3, plotlab, adj = 0)
for(j in 1:length(prop_set_short)){
...
Should also make plots showing each of the performance metrics at the max outplanting proportions and their cvs across outplanting proportions, or maybe just do that for the cases with the greatest variation in strategy scores?
...
lambda_R_setL <- seq(from = 0, to = 1.5, length.out = L2)*lambda_R0
...
# process the results
lambda_R2_dt <- orch_exp_sens_fun(sim_list = sens_lambda_R2, par_set2 = lambda_R_setL, L2, L1, max_yr, weights = list(reef_cover = 1, orch_fun = 1, reef_ROI = 1, tot_ROI = 1, tot_costs = 1, recruits_out = 1), dt_D0 = data.frame(NA))
...
maybe the question is how much survival and/or growth need to increase to make it "worth it" to keep the substrates in the lab for a certain amount of time?
