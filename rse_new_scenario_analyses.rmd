...
- should eventually turn the OHI plots into a function

```{r}
# now make ocean health index plots at 1 and 0
...
#dt_plot2 <- dt_plot %>% left_join(max_vals, by = c("par_rep")) %>% filter(is.na(reef_cover_max)==F) %>% filter(prop_out == prop_set[5]) # the proportion here should be the proportion with the highest score


dt_plot2 <- dt_plot %>% left_join(max_vals, by = c("par_rep")) %>% filter(is.na(reef_cover_max)==F) %>% filter(prop_out == opt_prop1) # the proportion here should be the proportion with the highest score

#View(dt_plot2)

# color for plot outlines
Lcol <- "gray"
...
```{r}
# now make ocean health index plots 

dt_plot <- orch_exp_5_D1
opt_prop1 <- orch_exp_5_D1_pmax
