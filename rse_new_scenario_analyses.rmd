...
orch_fun_ij <- model_summ(model_sim = sim_ij, location = "orchard", metric = "area_m2", n_reef = length(reef_treatments), n_orchard = length(orchard_treatments), n_lab = length(lab_treatments), size_classes = c(3, 4, 5))
    ...
    for(j in 1:L1){
     ...
     # dt_i$strategy_score[j] <- 1*ifelse(reef_cover_max_i == 0, NA, dt_i$reef_cover_mean[j]/reef_cover_max_i) + 1*dt_i$orch_function_mean[j]/orch_fun_max_i + 1*ifelse(reef_ROI_max_i == 0, NA, dt_i$reef_ROI_mean[j]/reef_ROI_max_i) + 1*dt_i$tot_ROI_mean[j]/tot_ROI_max_i + 1*dt_i$tot_costs[j]/tot_costs_max_i + 1*dt_i$reef_recruits_out_tot[j]/reef_recruits_out_tot_max_i
      
      w_sum <- sum(unlist(weights))
      
      dt_i$strategy_score[j] <- weights$reef_cover/w_sum*dt_i$reef_cover_mean[j]/reef_cover_max_i + weights$orch_fun/w_sum*dt_i$orch_function_mean[j]/orch_fun_max_i + weights$reef_ROI/w_sum*dt_i$reef_ROI_mean[j]/reef_ROI_max_i + weights$tot_ROI/w_sum*dt_i$tot_ROI_mean[j]/tot_ROI_max_i + weights$tot_costs/w_sum*(dt_i$tot_costs[j]/tot_costs_min_i)^(-1) + weights$recruits_out/w_sum*dt_i$reef_recruits_out_tot[j]/reef_recruits_out_tot_max_i
...
To Do
- repeat all the plots for 5 and 50 years with disturbance
- make a 4 panel plot (all 4 cases) for the strategy score plots
- make a 4 panel plot with the OHI plots
- think about if total costs should be money saved or something else where 1 is good

- then make the basic model dynamics plots. Make a new slide deck to put all the potential figures/SM figures for the paper
