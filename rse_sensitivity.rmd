...
lab_treatments <- c("0_T1") # lab treatments "TX" is tile type
...
surv_pars.r <- all_pars$surv_pars.r[[1]][[1]]
# reef parameters: growth
growth_pars.r <- all_pars$growth_pars.r[[1]][[1]]
# reef parameters: shrinkage
shrink_pars.r <- all_pars$shrink_pars.r[[1]][[1]]
# reef parameters: fragmentation
frag_pars.r <- all_pars$frag_pars.r[[1]][[1]]
...
pop_lambda_fun(surv_pars.r, all_pars$growth_pars.r[[1]][[1]], all_pars$shrink_pars.r[[1]][[1]], all_pars$frag_pars.r[[1]][[1]])
...
try making heatmaps in base R using adjacent polygons?

```{r}

# par(pty = "s")
# plot(x = c(1:5), y = c(1:5), col = NA, xlim = c(0, 5), ylim = c(0, 5), asp = 1, las = 1, xaxs = "i", yaxs = "i") # xaxs = "i" removes white space around the plot
# mtext(side = 1, "")
# #polygon(x = c(0, 1, 1, 0), y = c(0, 0, 1, 1))
# for(i in 1:5){ # x value
#   for(j in 1:5){ # y value
#     polygon(x = c(i-1, i, i, i-1), y = c(j-1, j-1, j, j), col = adjustcolor(col = "gray30", alpha.f = e95[i,j]/max(e95)))
#     text(x = i - 0.5, y = j - 0.5, as.character(round(e95[i,j], 2)))
#   }
# }
...
mtext(side = 3, expression(lambda~"="~0.95~"(mean parameters)"))
#polygon(x = c(0, 1, 1, 0), y = c(0, 0, 1, 1))
for(i in 1:5){ # x value
  for(j in 1:5){ # y value
...
mtext(side = 1, "Final size class", line = 1.75)
mtext(side = 2, "Initial size class", line = 1.5)
mtext(side = 3, expression(lambda~"="~1.3~"(95% quantile)"))
#polygon(x = c(0, 1, 1, 0), y = c(0, 0, 1, 1))
for(i in 1:5){ # x value
  for(j in 1:5){ # y value
...
mtext(side = 1, "Final size class", line = 1.75)
mtext(side = 2, "Initial size class", line = 1.5)
mtext(side = 3, expression(lambda~"="~0.6~"(5% quantile)"))
#polygon(x = c(0, 1, 1, 0), y = c(0, 0, 1, 1))
for(i in 1:5){ # x value
  for(j in 1:5){ # y value
...
# check that the rearrangement worked
# pop_matR <- t(pop_mat[5:1,])
# 
# par(pty = "s")
# plot(x = c(1:5), y = c(1:5), col = NA, xlim = c(0, 5), ylim = c(0, 5), asp = 1, las = 1, xaxs = "i", yaxs = "i") # xaxs = "i" removes white space around the plot
# mtext(side = 1, "")
...
#     text(x = i - 0.5, y = j - 0.5, as.character(round(pop_matR[i,j], 2)))
#   }
# }
...
start with a growing population and see which parameter changes will cause it to die
...
lambda_R <- 1255111 # mean number of larvae produced by reference reefs each year
...
s0[,1] <- rep(0.95, years)*0
s0[,2] <- rep(0.95, years)*0

# fractions of settled larvae in each lab treatment that survive to 1 yr outplanting
s1 <- matrix(NA, nrow = years, ncol = 2)
s1[,1] <- rep(0.7, years)*0
s1[,2] <- rep(0.7, years)*0

m0 <- c(0.02) # density dependent mortality rate of larvae in each lab treatment that get outplanted immediately
m1 <- c(0.02) # density dependent mortality rate of larvae in each lab treatment that get outplanted after 1 year
...
lab_max <- 4000 # total number of settlers that the lab can accommodate
...
orchard_size <- c(30*500) # number of individuals each orchard treatment has space for
...
pop_mat <-  (Tmat.i + Fmat.i) %*% diag(surv_pars.r[[1]][[1]])
...
# start with percent decrease in survival: from 0 to 1
#all_pars$surv_pars.r[[1]][[1]]

# percent decrease in growth: from 0 to 1
#all_pars$growth_pars.r[[1]][[1]]

# percent increase in shrinkage...except a lot are starting at 0
#all_pars$shrink_pars.r[[1]][[1]]

# percent decrease in fragmentation
#all_pars$frag_pars.r[[1]][[1]]

# maybe start with percent decrease in survival, and then add on top of that single changes in the other parameters?
perc_set <- seq(from = 0, to = 1, length.out = 100)
lambda_surv_mat95 <- matrix(NA, nrow = length(perc_set), ncol = 5)

for(j in 1:length(perc_set)){
  
  for(k in 1:5){
    
    #surv_j <- rep(1, 5) 
    surv_j <- all_pars$surv_pars.r[[1]][[1]]
    surv_j[k] <- surv_j[k]*perc_set[j]
    
    lambda_surv_mat95[j,k] <- pop_lambda_fun(surv_j, all_pars$growth_pars.r[[1]][[1]], all_pars$shrink_pars.r[[1]][[1]], all_pars$frag_pars.r[[1]][[1]])
...
ylim1 <- min(lambda_surv_mat95, na.rm = T)
ylim2 <- max(lambda_surv_mat95, na.rm = T)

plot(x = rev(perc_set), y = lambda_surv_mat95[,1], col=NA, type = "l", las = 1, ylim = c(ylim1, ylim2*1.01), xlab = NA, ylab = NA)
mtext(side = 1, "Proportional decrease in survival", line = 2.5)
mtext(side = 2, expression("Population"~lambda), line = 2.5)
for(k in 1:5){
  lines(x = rev(perc_set), y = lambda_surv_mat95[,k], col = cols1[k], lwd = 2)
}
#abline(h = growth_min, lty = 3)
abline(h = 1, lty = 3)
legend("bottomleft", legend = c("1", "2", "3", "4", "5"), col = cols1, lwd = 2, title = "Size class")
...
N0.r <- list()
N0.r[[1]] <- list() # second reef subpop
N0.r[[1]][[1]] <- c(5, 15, 15, 10, 5)#rep(0, n) # first source (external recruits)
N0.r[[1]][[2]] <- rep(0, n) # second source
...
```{r}
# also plot histogram of time to population extinction for the lambda values less than one

hist(log(1/sum(N0.r[[1]][[1]]), base = lambda_set[which(lambda_set < 0.998)]), main = NA, xlab = NA, ylab = NA, las = 1, breaks = 20)
mtext(side = 1, "Years to extinction when N0 = 50", line = 2)
mtext(side = 2, "Frequency", line = 2)


hist(log(1/sum(N0.r[[1]][[1]]), base = lambda_set[which(lambda_set < 1)]), main = NA, xlab = NA, ylab = NA, las = 1, breaks = 10)
mtext(side = 1, "Years to extinction when N0 = 50", line = 2)
mtext(side = 2, "Frequency", line = 2)

sort(log(1/sum(N0.r[[1]][[1]]), base = lambda_set[which(lambda_set < 1)]))
sort(lambda_set[which(lambda_set < 1)])
```
...
# update all inputs that involve length of simulation
s0 <- matrix(NA, nrow = years2, ncol = 2)
s0[,1] <- rep(0.95, years2)*0
s0[,2] <- rep(0.95, years2)*0

# fractions of settled larvae in each lab treatment that survive to 1 yr outplanting
s1 <- matrix(NA, nrow = years2, ncol = 2)
s1[,1] <- rep(0.7, years2)*0
s1[,2] <- rep(0.7, years2)*0
...
## check for patterns in lambda < 1 parameter sets
...
lambda <- 1000 #1255111 # external recruitment to reef
...
# Restoring diff size classes

Compare effects of adding babies vs. larger colonies. Cheat and just use the same reference reef-to-lab-to intervention reef pipeline for this, but change the size class of the corals collected from the reef (size_props in the lab parameters) and the number of corals collected from the reference reef (lambda_R)
...
m0 <- c(0.02)*0 # density dependent mortality rate of larvae in each lab treatment that get outplanted immediately
m1 <- c(0.02)*0 # density dependent mortality rate of larvae in each lab treatment that get outplanted after 1 year
...
# number of corals to outplant this year
lambda_R <- 10000#1255111 # mean number of larvae produced by reference reefs each year

sim1 <- rse_mod1(years, n, A_mids, surv_pars.r, dens_pars.r, growth_pars.r, shrink_pars.r, frag_pars.r, fec_pars.r, surv_pars.o, dens_pars.o, growth_pars.o, shrink_pars.o, frag_pars.o, fec_pars.o, lambda, lambda_R, sigma_s, sigma_f, ext_rand, seeds, dist_yrs, dist_pars.r, dist_effects.r, dist_pars.o, dist_effects.o, orchard_treatments, reef_treatments, lab_treatments, lab_pars, rest_pars, N0.r, N0.o, N0.l)


max(sim1$orchard_pops[[1]][[1]])

unique(sim1$reef_out[[1]][[1]])

unique(sim1$reef_out[[1]][[2]]) # even if all mortality and density dependence is turned off, this might not match lambda_R if lambda_R is so big that tank_max starts to cap larvae
...
# outplanting size classes 1, 2, 3, 4, and 5... maybe go down by factor of 10 each time? 10000, 1000, 100, 10, 1
# update: outplant the same area each year (which will be different numbers of individuals)
# per-coral area in each size class in m2: A_mids*0.0001
...
area_out <- 500*0.337 # 168.5; round up to 170
...
surv_pars.r[[1]][[1]]*lambda_R_12345#*A_mids*0.0001


# final size distributions
end1 <- mod_out_L[[1]]$reef_pops_pre[[1]][[2]][,years]
end2 <- mod_out_L[[2]]$reef_pops_pre[[1]][[2]][,years]
end3 <- mod_out_L[[3]]$reef_pops_pre[[1]][[2]][,years]
end4 <- mod_out_L[[4]]$reef_pops_pre[[1]][[2]][,years]
end5 <- mod_out_L[[5]]$reef_pops_pre[[1]][[2]][,years]

end_mat <- matrix(c(end1, end2, end3, end4, end5), nrow = 5, ncol = 5, byrow = F)

end_mat

```

I think it's just that numbers matter - if you look at final size distributions, you end up with a larger number of large corals if you outplant thousands of size class two corals than if you just outplant a hundred or so giant corals. Basically you can get more large corals by outplanting a ton of little corals than by outplanting a few large corals, provided enough of those little corals survive
...
plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end2))), xlim = c(0.5, 5.5), las = 1)
for(i in 1:5){
...
# repeat but everything scaled to sum to 1


# end11 <- mod_out_L[[1]]$reef_pops_pre[[1]][[2]][,years]/sum(end1)
# end21 <- mod_out_L[[2]]$reef_pops_pre[[1]][[2]][,years]/sum(end2)
# end31 <- mod_out_L[[3]]$reef_pops_pre[[1]][[2]][,years]/sum(end3)
# end41 <- mod_out_L[[4]]$reef_pops_pre[[1]][[2]][,years]/sum(end4)
# end51 <- mod_out_L[[5]]$reef_pops_pre[[1]][[2]][,years]/sum(end5)
# 
# end_mat1 <- matrix(c(end11, end21, end31, end41, end51), nrow = 5, ncol = 5, byrow = F)
# 
# plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, 1), xlim = c(0.5, 5.5), las = 1)
# for(i in 1:5){
#   polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat1[,i]), sum(end_mat1[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
#   for(k in 1:5){
#     
#     if(k == 1){
#       polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, end_mat1[k,i], end_mat1[k,i]), col = cols1[k])
#     } else{
#       polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(sum(end_mat1[1:(k-1),i]), sum(end_mat1[1:(k-1),i]), sum(end_mat1[1:(k-1),i]) + end_mat1[k,i], sum(end_mat1[1:(k-1),i]) + end_mat1[k,i]), col = cols1[k])
#     }
#     
#   }
#   
...
repeat but outplanting the same number of each size class

```{r}
lambda_R_12345 <- rep(200, 5)
out_sizes <- c(1, 2, 3, 4, 5)
...
end1 <- mod_out_L2[[1]]$reef_pops_pre[[1]][[2]][,years]
end2 <- mod_out_L2[[2]]$reef_pops_pre[[1]][[2]][,years]
end3 <- mod_out_L2[[3]]$reef_pops_pre[[1]][[2]][,years]
end4 <- mod_out_L2[[4]]$reef_pops_pre[[1]][[2]][,years]
end5 <- mod_out_L2[[5]]$reef_pops_pre[[1]][[2]][,years]

end_mat2 <- matrix(c(end1, end2, end3, end4, end5), nrow = 5, ncol = 5, byrow = F)
...
plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_ind2)), col = NA, xlab = NA, ylab = NA, las = 1)
for(j in 1:5){
  lines(x = c(1:years), y = mod_ind2[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}

plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area2)), col = NA, xlab = NA, ylab = NA, las = 1)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area2[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}

plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end5))), xlim = c(0.5, 5.5), las = 1)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat2[,i]), sum(end_mat2[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
    
    if(k == 1){
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, end_mat2[k,i], end_mat2[k,i]), col = cols1[k])
    } else{
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(sum(end_mat2[1:(k-1),i]), sum(end_mat2[1:(k-1),i]), sum(end_mat2[1:(k-1),i]) + end_mat2[k,i], sum(end_mat2[1:(k-1),i]) + end_mat2[k,i]), col = cols1[k])
    }
...
main text: just the fixed area simulations

```{r}
# plot

pdf("TempFigs/outplanting_size_main.pdf", height = 4, width = 8)

par(mfrow = c(1,2))
layout(matrix(c(1,2), nrow = 1, ncol = 2, byrow = T), widths = rep(1, 2), heights = rep(1, 1))
#layout.show(2)
par(mar=c(0, 5, 2, 0), oma = c(4, 0.1, 0.1, 0.25))


plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area)), col = NA, xlab = NA, ylab = NA, las = 1)
#mtext(side = 3, expression("a) Outplant 170"*m^2~"each"), adj = 0)
mtext(side = 3, expression("a) Outplant 170"*m^2~"each year"), adj = 0)
mtext(side = 1, "Year", line = 2.5)
mtext(side = 2, expression("Coral cover ("*m^2*")"), line = 3)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}
legend("topleft", legend = c("1 (337,000)", "2 (30,636)", "3 (3,370)", "4 (688)", "5 (181)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9)


plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end_mat[,2]))), xlim = c(0.5, 5.5), las = 1, ylab = NA)
mtext(side = 1, "Size class outplanted", line = 2.5)
mtext(side = 3, "b) Final size distributions", adj = 0)
mtext(side = 2, "Number of coral individuals", line = 3.75)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat[,i]), sum(end_mat[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
...
# pdf("TempFigs/outplanting_size.pdf", height = 6, width = 5)
# 
# par(mfrow = c(1, 2))
# layout(matrix(c(1, 2), nrow = 2, ncol = 1, byrow = T), widths = rep(1, 1), heights = rep(1, 2))
# #layout.show(2)
# par(mar=c(0, 5, 2, 0.1), oma = c(4, 2, 0.1, 0.1))
# 
# 
# plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area2)), col = NA, xlab = NA, ylab = NA, las = 1, xaxt = "n")
# mtext(side = 3, "a) Outplant 200 individuals each", adj = 0)
# axis(side = 1, at = c(0, 10, 20, 30, 40, 50), labels = NA)
# for(j in 1:5){
#   lines(x = c(1:years), y = mod_area2[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
# }
# legend("topleft", legend = c("1 (200)", "2 (200)", "3 (200)", "4 (200)", "5 (200)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9)
# 
# 
# plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area)), col = NA, xlab = NA, ylab = NA, las = 1)
# mtext(side = 3, expression("b) Outplant 170"*m^2~"each"), adj = 0)
# mtext(side = 1, "Year", line = 2.5)
# mtext(side = 2, expression("Coral cover ("*m^2*")"), line = -2, outer = T)
# for(j in 1:5){
#   lines(x = c(1:years), y = mod_area[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
# }
# legend("topleft", legend = c("1 (337,000)", "2 (30,636)", "3 (3,370)", "4 (688)", "5 (181)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9)
...
pdf("TempFigs/outplanting_size.pdf", height = 6, width = 7)

par(mfrow = c(2, 2))
layout(matrix(c(1,2,3,4), nrow = 2, ncol = 2, byrow = F), widths = rep(1, 2), heights = rep(1, 2))
#layout.show(2)
par(mar=c(0, 5, 2, 0), oma = c(4, 0.25, 0.1, 0.25))
...
plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area)), col = NA, xlab = NA, ylab = NA, las = 1)
mtext(side = 3, expression("c) Outplant 170"*m^2~"each"), adj = 0)
mtext(side = 1, "Year", line = 2.5)
mtext(side = 2, expression("Coral cover ("*m^2*")"), line = -2, outer = T)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
...
# fixed number
plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end_mat2[,5]))), xlim = c(0.5, 5.5), las = 1, ylab = NA, xaxt = "n")
mtext(side = 3, "b) Final size distributions from a)", adj = 0)
mtext(side = 2, "Number of coral individuals", line = 3.75, adj = -3)
axis(side = 1, at = c(1, 2, 3, 4, 5), labels = NA)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat2[,i]), sum(end_mat2[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
...
plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end_mat[,2]))), xlim = c(0.5, 5.5), las = 1, ylab = NA)
mtext(side = 1, "Size class outplanted", line = 2.5)
mtext(side = 3, "d) Final size distributions from c)", adj = 0)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat[,i]), sum(end_mat[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
...
repeat everything with equal survival
...
# equal survival
surv_pars.rE <- surv_pars.r
surv_pars.rE[[1]][[1]] <- rep(mean(surv_pars.r[[1]][[1]]), 5)
surv_pars.rE[[1]][[2]] <- rep(mean(surv_pars.r[[1]][[1]]), 5)

# test turning off fragmentation
# frag_pars.r0 <- frag_pars.r
# frag_pars.r0[[1]][[2]][[4]] <- 0*frag_pars.r0[[1]][[2]][[4]]
# frag_pars.r0[[1]][[2]][[5]] <- 0*frag_pars.r0[[1]][[2]][[5]]

# holding list for model output
mod_out_L_eq <- list()

mod_ind_eq <- matrix(NA, nrow = years, ncol = 5) # total number of individuals on reef
mod_area_eq <- matrix(NA, nrow = years, ncol = 5) # total area covered on the reef
...
mod_out_L_eq[[j]] <- sim_j


mod_ind_eq[,j] <- model_summ(model_sim = sim_j, location = "reef", metric = "ind", n_reef = length(reef_treatments), n_orchard = length(orchard_treatments), n_lab = length(lab_treatments))

mod_area_eq[,j] <- model_summ(model_sim = sim_j, location = "reef", metric = "area_m2", n_reef = length(reef_treatments), n_orchard = length(orchard_treatments), n_lab = length(lab_treatments))


}

surv_pars.rE[[1]][[1]]*lambda_R_12345#*A_mids*0.0001


# final size distributions
end1 <- mod_out_L_eq[[1]]$reef_pops_pre[[1]][[2]][,years]
end2 <- mod_out_L_eq[[2]]$reef_pops_pre[[1]][[2]][,years]
end3 <- mod_out_L_eq[[3]]$reef_pops_pre[[1]][[2]][,years]
end4 <- mod_out_L_eq[[4]]$reef_pops_pre[[1]][[2]][,years]
end5 <- mod_out_L_eq[[5]]$reef_pops_pre[[1]][[2]][,years]

end_mat_eq <- matrix(c(end1, end2, end3, end4, end5), nrow = 5, ncol = 5, byrow = F)
...
mod_ind2_eq <- matrix(NA, nrow = years, ncol = 5) # total number of individuals on reef
mod_area2_eq <- matrix(NA, nrow = years, ncol = 5) # total area covered on the reef
...
mod_out_L2_eq[[j]] <- sim_j


mod_ind2_eq[,j] <- model_summ(model_sim = sim_j, location = "reef", metric = "ind", n_reef = length(reef_treatments), n_orchard = length(orchard_treatments), n_lab = length(lab_treatments))

mod_area2_eq[,j] <- model_summ(model_sim = sim_j, location = "reef", metric = "area_m2", n_reef = length(reef_treatments), n_orchard = length(orchard_treatments), n_lab = length(lab_treatments))


}


end1 <- mod_out_L2_eq[[1]]$reef_pops_pre[[1]][[2]][,years]
end2 <- mod_out_L2_eq[[2]]$reef_pops_pre[[1]][[2]][,years]
end3 <- mod_out_L2_eq[[3]]$reef_pops_pre[[1]][[2]][,years]
end4 <- mod_out_L2_eq[[4]]$reef_pops_pre[[1]][[2]][,years]
end5 <- mod_out_L2_eq[[5]]$reef_pops_pre[[1]][[2]][,years]

end_mat2_eq <- matrix(c(end1, end2, end3, end4, end5), nrow = 5, ncol = 5, byrow = F)
...
pdf("TempFigs/outplanting_sizeE.pdf", height = 6, width = 7)
...
plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area2_eq)), col = NA, xlab = NA, ylab = NA, las = 1, xaxt = "n")
mtext(side = 3, "a) Outplant 200 individuals each", adj = 0)
axis(side = 1, at = c(0, 10, 20, 30, 40, 50), labels = NA)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area2_eq[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}
legend("topright", legend = c("1 (200)", "2 (200)", "3 (200)", "4 (200)", "5 (200)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9, inset = c(0,0.1))


plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area_eq)), col = NA, xlab = NA, ylab = NA, las = 1)
mtext(side = 3, expression("c) Outplant 170"*m^2~"each"), adj = 0)
mtext(side = 1, "Year", line = 2.5)
mtext(side = 2, expression("Coral cover ("*m^2*")"), line = -2, outer = T)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area_eq[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}
legend("topright", legend = c("1 (337,000)", "2 (30,636)", "3 (3,370)", "4 (688)", "5 (181)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9, inset = c(0,0.1))

# final distributions

# fixed number
plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end_mat2_eq[,5]))), xlim = c(0.5, 5.5), las = 1, ylab = NA, xaxt = "n")
mtext(side = 3, "b) Final size distributions from a)", adj = 0)
mtext(side = 2, "Number of coral individuals", line = 3.5, adj = -3)
axis(side = 1, at = c(1, 2, 3, 4, 5), labels = NA)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat2_eq[,i]), sum(end_mat2_eq[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
    
    if(k == 1){
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, end_mat2_eq[k,i], end_mat2_eq[k,i]), col = cols1[k])
    } else{
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(sum(end_mat2_eq[1:(k-1),i]), sum(end_mat2_eq[1:(k-1),i]), sum(end_mat2_eq[1:(k-1),i]) + end_mat2_eq[k,i], sum(end_mat2_eq[1:(k-1),i]) + end_mat2_eq[k,i]), col = cols1[k])
    }
...
# fixed area
plot(x = c(1, 2, 3, 4, 5), y = rep(0, 5), col = NA, ylim = c(0, max(sum(end_mat_eq[,1]))), xlim = c(0.5, 5.5), las = 1, ylab = NA)
mtext(side = 1, "Size class outplanted", line = 2.5)
mtext(side = 3, "d) Final size distributions from c)", adj = 0)
for(i in 1:5){
  polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, sum(end_mat_eq[,i]), sum(end_mat_eq[,i])), col = adjustcolor(col = "gray", alpha.f = 0))
  for(k in 1:5){
    
    if(k == 1){
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(0, 0, end_mat_eq[k,i], end_mat_eq[k,i]), col = cols1[k])
    } else{
      polygon(x = c(i-0.25, i + 0.25, i + 0.25, i-0.25), y = c(sum(end_mat_eq[1:(k-1),i]), sum(end_mat_eq[1:(k-1),i]), sum(end_mat_eq[1:(k-1),i]) + end_mat_eq[k,i], sum(end_mat_eq[1:(k-1),i]) + end_mat_eq[k,i]), col = cols1[k])
    }
...
```
note: If you turn off fragmentation, for the outplanting fixed number case you still get higher area when outplanting size class 5 (because they are larger), but you get similar total numbers of individuals across all outplanting size classes
