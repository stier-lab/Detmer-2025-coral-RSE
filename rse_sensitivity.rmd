...
# update: outplant the same area (10m2) each year (which will be different numbers of individuals)
# per-coral area in each size class
#A_mids*0.0001
lambda_R_12345 <- round(10/(A_mids*0.0001)) #c(10000, 1000, 100, 10, 1)
...
```{r}
lambda_R_12345 <- c(100, 100, 100, 100, 100)
out_sizes <- c(1, 2, 3, 4, 5)
...
mtext(side = 3, "a) Outplant 100 individuals each", adj = 0)
axis(side = 1, at = c(0, 10, 20, 30, 40, 50), labels = NA)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area2[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}
legend("topleft", legend = c("1 (100)", "2 (100)", "3 (100)", "4 (100)", "5 (100)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9)


plot(x = c(1:years), y = rep(0, years), ylim = c(0, max(mod_area)), col = NA, xlab = NA, ylab = NA, las = 1)
mtext(side = 3, expression("b) Outplant 10"*m^2~"each"), adj = 0)
mtext(side = 1, "Year", line = 2.5)
mtext(side = 2, expression("Coral cover ("*m^2*")"), line = -2, outer = T)
for(j in 1:5){
  lines(x = c(1:years), y = mod_area[,j], type = "l", lty = j, col = cols1[j], lwd = 2)
}
legend("topleft", legend = c("1 (20,000)", "2 (1,818)", "3 (200)", "4 (41)", "5 (11)"), col = cols1, lty = c(1:5), lwd = 2, bty = "n", ncol = 1, cex = 0.9)

dev.off()

round(10/(A_mids*0.0001))
