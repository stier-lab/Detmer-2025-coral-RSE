...
surv_pars.r[[1]][[2]] <- c(0.016, 0.67, 0.74, 0.89, 0.93) # survival probabilities for second source of recruits to first reef subpop (first lab treatment = cement)
#surv_pars.r[[1]][[3]] <- c(0.016, 0.67, 0.74, 0.89, 0.93) # survival probabilities for third source of recruits to first reef subpop (second lab treatment = ceramic)
...
shrink_pars.r[[1]][[1]] <- list(NULL, c(0.066), c(0.034, 0.15), c(0.024, 0.021, 0.15), c(0.007, 0.001, 0.0132, 0.0998))
...
shrink_pars.o[[1]][[1]] <- list(NULL, c(0.01), c(0.034, 0.15), c(0.024, 0.021, 0.15), c(0.007, 0.001, 0.0132, 0.0998)) 
...
reef_prop <- c(0.75) # proportion of lab recruits from each lab treatment outplanted to reef (1-proportion outplanted to orchard)
...
plot(x = cap_set1, y = area_mat1[,1], type = "l", las = 1, col = col_set[3], lwd = 2, xlab = NA, ylab = NA, ylim = c(0, 100))
