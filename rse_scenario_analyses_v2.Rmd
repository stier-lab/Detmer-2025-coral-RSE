...
surv_pars.r[[1]][[2]] <- c(0.01, 0.44, 0.53, 0.70, 0.84) # survival probabilities for second source of recruits to first reef subpop (first lab treatment = cement)
...
lambda_R <- 3350000 # mean number of larvae produced by reference reefs each year
...
text(x = years-1, y = max(reef_100p)-0.05*max(reef_100p), paste("$", as.character(tot_100), "/year", sep = ""), col = col_set[1])
text(x = years-1, y = max(reef_50p)-0.1*max(reef_50p), paste("$", as.character(tot_50), "/year", sep = ""), col = col_set[2])
text(x = years-1, y = max(reef_0p) + 0.25*max(reef_0p), paste("$", as.character(tot_0), "/year", sep = ""), col = col_set[3])
...
mtext(side = 2, expression("Coral Cover ("*m^2*")"), line = 2.5, cex = 1.1)
...
orchard_treatments <- c("orchard1") # orchards (could have multiple orchards and/or orchards with different post-outplanting treatments)
reef_treatments <- c("reef1") # reef treatments/subpopulations (could have "urchin outplanting" or "algal removal" or other postoutplanting treatments)
lab_treatments <- c("0_T1", "1_T1") # lab (cement = T1, ceramic = T2). "TX" is tile type, "X_" indicates whether recruits are outplanted immediately (0_) or the next year (0_1)
...
surv_pars.r[[1]][[1]] <- c(0.01, 0.44, 0.53, 0.70, 0.84) # survival probabilities for first source of recruits to first reef subpop (external recruits)
surv_pars.r[[1]][[2]] <- c(0.01, 0.44, 0.53, 0.70, 0.84) # survival probabilities for second source of recruits to first reef subpop (first lab treatment = cement)
surv_pars.r[[1]][[3]] <- c(0.01, 0.44, 0.53, 0.70, 0.84) # survival probabilities for third source of recruits to first reef subpop (second lab treatment = ceramic)
# if there were more reef treatments/subpopulations:
# surv_pars.r[[2]] <- list(), then define surv_pars.r[[2]][[1]], [[2]][[2]], [[2]][[3]]
...
size_props <- matrix(NA, nrow = length(lab_treatments), ncol = n) # matrix for fractions of retained recruits in each size class at the end of their year in the lab
size_props[1, ] <- c(1, 0, 0, 0, 0) # first lab treatment (0_T1)
size_props[2, ] <- c(0.75, 0.25, 0, 0, 0) # second lab treatment (0_T2)
lab_pars <- list(s0 = s0, s1 = s1, m0 = m0, m1 = m1, sett_props = sett_props, size_props = size_props)
...
-- third zero: population gets its first outplants, so the population now has some recruits, but the pre population doesn't because these recruits don't appear until after outplanting 
...
rest_pars.100 <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max.100, orchard_size = orchard_size, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)

T2_100 <- rse_mod1(years, n, A_mids, surv_pars.r, dens_pars.r, growth_pars.r, shrink_pars.r, frag_pars.r, fec_pars.r, surv_pars.o, dens_pars.o, growth_pars.o, shrink_pars.o, frag_pars.o, fec_pars.o, lambda, lambda_R, sigma_s, sigma_f, ext_rand, seeds, dist_yrs, dist_pars.r, dist_effects.r, dist_pars.o, dist_effects.o, orchard_treatments, reef_treatments, lab_treatments, lab_pars, rest_pars.100, N0.r, N0.o, N0.l)

lab_retain_max.50 <- c(500) # max settlers retained for a year

rest_pars.50 <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max.50, orchard_size = orchard_size, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)

T2_50 <- rse_mod1(years, n, A_mids, surv_pars.r, dens_pars.r, growth_pars.r, shrink_pars.r, frag_pars.r, fec_pars.r, surv_pars.o, dens_pars.o, growth_pars.o, shrink_pars.o, frag_pars.o, fec_pars.o, lambda, lambda_R, sigma_s, sigma_f, ext_rand, seeds, dist_yrs, dist_pars.r, dist_effects.r, dist_pars.o, dist_effects.o, orchard_treatments, reef_treatments, lab_treatments, lab_pars, rest_pars.50, N0.r, N0.o, N0.l)

lab_retain_max.0 <- c(0) # max settlers retained for a year

rest_pars.0 <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max.0, orchard_size = orchard_size, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)
...
text(x = years-1, y = max(reef_100p)-0.05*max(reef_100p), paste("$", "xx", "/year", sep = ""), col = col_set[1])
text(x = years-1, y = max(reef_50p)+0.11*max(reef_50p), paste("$", "xx", "/year", sep = ""), col = col_set[2])
text(x = years-1, y = max(reef_0p) + 0.95*max(reef_0p), paste("$", "xx", "/year", sep = ""), col = col_set[3])
lines(x = c(1:years), y = reef_50p, type = "l", col = col_set[2], lwd = 2)
lines(x = c(1:years), y = reef_0p, type = "l", col = col_set[3], lwd = 2)
...
assume it costs x dollars per year to keep a recruit in the lab

but then money invested is perfectly correlated with lab max, so really it would be more interesting to compare keeping them in the lab vs. some other strategy, but for now just repeat for multiple growth rate parameters (slices of the heatmap)
...
growth_set <- c(0, 0.25, 0.5)
max_set <- seq(from = 0, to = 500, length.out = 25)
...
#cost_mat <- matrix(NA, nrow = length(growth_set), ncol = length(max_set))
...
lab_pars.ij <- list(s0 = s0, s1 = s1, m0 = m0, m1 = m1, sett_props = sett_props, size_props = size_props.ij)

lab_retain_max.ij <- max_set[j]
    
rest_pars.ij <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max.ij, orchard_size = orchard_size, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)
  ...
  # record lab population size (used to calculate costs)
 # cost_mat[i,j] <- sum(sim.ij$lab_pops[[2]])*cost_per_recruit
...
plot(x = max_set, y = area_mat.5[1,], type = "l", las = 1, col = col_set[3], lwd = 2, xlab = NA, ylab = NA, ylim = c(0, 1.75))
mtext(side = 1, "$ invested in lab", line = 2, cex = 1.1)
mtext(side = 2, expression("Coral Cover ("*m^2*")"), line = 2, cex = 1.1)
lines(x = max_set, y = area_mat.5[2,], type = "l", col = col_set[2], lwd = 2)
lines(x = max_set, y = area_mat.5[3,], type = "l", col = col_set[1], lwd = 2)
legend("topleft", legend = c("50% growth", "25% growth", "no growth"), lwd = 2, col = col_set, bty = "n")


```

this is assuming that costs are proportional to the max number of recruits that can be stored in the lab
...
surv_pars.o <- list()
surv_pars.o[[1]] <- list() # first treatment
surv_pars.o[[1]][[1]] <- c(0.010, 0.760, 0.870, 0.950, 0.999)#c(0.010, 0.44, 0.53, 0.70, 0.84) # first source (first lab treatment)
surv_pars.o[[1]][[2]] <- surv_pars.o[[1]][[1]] # second source (second lab treatment)
...
orchard_size <- c(100000) # number of individuals each orchard treatment has space for
...
plot(x = c(1:years), y = reef_100p, type = "l", las = 1, col = col_set[1], lwd = 2, xlab = NA, ylab = NA, ylim = c(0, 0.25))
...
cap_set1 <- seq(from = 0, to = 500, length.out = 25) # max orchard capacity

pop_mat1 <- matrix(NA, nrow = length(cap_set), ncol = length(prop_set))
#pcover_mat <- matrix(NA, nrow = length(inv_set), ncol = length(prop_set))
area_mat1 <- matrix(NA, nrow = length(cap_set), ncol = length(prop_set))
...
rest_pars.ij <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop.ij, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max, orchard_size = orchard_size.ij, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)
  ...
  # record final population size
  pop_mat1[i,j] <- sum(sim.ij$reef_pops_pre[[1]][[1]][,years]) + sum(sim.ij$reef_pops_pre[[1]][[2]][,years])
  
  # record area covered
  area_mat1[i,j] <- (sum(sim.ij$reef_pops_pre[[1]][[1]][,years]*A_mids) + sum(sim.ij$reef_pops_pre[[1]][[2]][,years]*A_mids))/10000
...
plot(x = cap_set1, y = area_mat1[1,], type = "l", las = 1, col = col_set[3], lwd = 2, xlab = NA, ylab = NA, ylim = c(0, 0.3))
mtext(side = 1, "investment in orchard", line = 2, cex = 1.1)
mtext(side = 2, expression("Coral Cover ("*m^2*")"), line = 2.5, cex = 1.1)
lines(x = cap_set1, y = area_mat1[2,], type = "l", col = col_set[2], lwd = 2)
lines(x = cap_set1, y = area_mat1[3,], type = "l", col = col_set[1], lwd = 2)
legend("topleft", legend = c("0% to reef", "50% to reef", "100% to reef"), lwd = 2, col = col_set, bty = "n")


```

remember the surplus always goes to the reef, and the bigger the orchard is the lower the surplus
...
rest_pars.ij <- list(tile_props = tile_props, orchard_yield = orchard_yield, reef_yield = reef_yield, reef_prop = reef_prop.ij, reef_out_props = reef_out_props, orchard_out_props = orchard_out_props, reef_areas = reef_areas, lab_max = lab_max, lab_retain_max = lab_retain_max, orchard_size = orchard_size.ij, transplant = transplant, trans_mats = trans_mats, trans_reef = trans_reef)
  ...
  # record final population size
  pop_mat[i,j] <- sum(sim.ij$reef_pops_pre[[1]][[1]][,years]) + sum(sim.ij$reef_pops_pre[[1]][[2]][,years])
  
  # record area covered
  area_mat[i,j] <- (sum(sim.ij$reef_pops_pre[[1]][[1]][,years]*A_mids) + sum(sim.ij$reef_pops_pre[[1]][[2]][,years]*A_mids))/10000
...
filled.contour(x = prop_set, y = cap_set, area_mat, key.title = title(main = expression("Cover ("*m^2*")"), cex.main = 0.8, line = 0.5),
  ...
  axis(1)
  axis(2)
  contour(x = prop_set, y = cap_set, area_mat, levels = 0.25, lwd = 2, drawlabels = F, add = T)})
