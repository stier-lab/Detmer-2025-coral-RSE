...
surv_pars.r[[2]][[3]] <- c(0.015, 0.44, 0.53, 0.70, 0.84) # survival probabilities for third source of recruits to second reef subpop (second lab treatment)
...
frag_pars.r[[2]][[1]] <- list(NULL, c(0), c(0, 0), c(0, 0, 0), c(0, 0, 0, 0)) # first source
...
surv_pars.r[[2]][[2]] <- c(0.001, 0.44, 0.53, 0.70, 0.84) # survival probabilities for second source of recruits to second reef subpop (first lab treatment)
surv_pars.r[[2]][[3]] <- c(0.005, 0.44, 0.53, 0.70, 0.84) # survival probabilities for third source of recruits to second reef subpop (second lab treatment)
...
reef1.5 <- rep(NA, length(par_set))
reef2.5 <- rep(NA, length(par_set))

# reef population sizes at 10 years
reef1.10 <- rep(NA, length(par_set))
reef2.10 <- rep(NA, length(par_set))

# cumulative number of recruits outplanted
reef_out2.5 <- rep(NA, length(par_set))
reef_out2.10 <- rep(NA, length(par_set))


# intervention reef reproductive output at 5 years
rep1.5 <- rep(NA, length(par_set))
rep2.5 <- rep(NA, length(par_set))

# intervention reef reproductive output at 10 years
rep1.10 <- rep(NA, length(par_set))
rep2.10 <- rep(NA, length(par_set))
...
# save the results
reef1.5[i] <- sum(sim.i$reef_pops_pre[[1]][[1]][,5])
reef2.5[i] <- sum(sim.i$reef_pops_pre[[2]][[1]][,5]) + sum(sim.i$reef_pops_pre[[2]][[2]][,5]) + sum(sim.i$reef_pops_pre[[2]][[3]][,5])
reef1.10[i] <- sum(sim.i$reef_pops_pre[[1]][[1]][,10])
reef2.10[i] <- sum(sim.i$reef_pops_pre[[2]][[1]][,10]) + sum(sim.i$reef_pops_pre[[2]][[2]][,10]) + sum(sim.i$reef_pops_pre[[2]][[3]][,10])

# cumulative numbers of recruits outplanted
reef_out2.5[i] <- sum(sim.i$reef_out[[2]][[2]][2:5]) + sum(sim.i$reef_out[[2]][[3]][2:5])
reef_out2.10[i] <- sum(sim.i$reef_out[[2]][[2]][2:10]) + sum(sim.i$reef_out[[2]][[3]][2:10])

# intervention reef reproductive output at 5 years
rep1.5[i] <- sum(sim.i$reef_pops_pre[[1]][[1]][,5]*fec_pars.r[[1]][[1]])
rep2.5[i] <- sum(sim.i$reef_pops_pre[[2]][[1]][,5]*fec_pars.r[[2]][[1]]) + sum(sim.i$reef_pops_pre[[2]][[2]][,5]*fec_pars.r[[2]][[2]]) + sum(sim.i$reef_pops_pre[[2]][[3]][,5]*fec_pars.r[[2]][[3]])

# intervention reef reproductive output at 10 years
rep1.10[i] <- sum(sim.i$reef_pops_pre[[1]][[1]][,10]*fec_pars.r[[1]][[1]])
rep2.10[i] <- sum(sim.i$reef_pops_pre[[2]][[1]][,10]*fec_pars.r[[2]][[1]]) + sum(sim.i$reef_pops_pre[[2]][[2]][,10]*fec_pars.r[[2]][[2]]) + sum(sim.i$reef_pops_pre[[2]][[3]][,10]*fec_pars.r[[2]][[3]])
...
plot(x = T1_props, y = reef2.5, type = "l", ylim = c(0, 3), las = 1)
lines(x = T1_props, y = reef2.10, lty = 2)

plot(x = T1_props, y = reef_out2.5, type = "l", las = 1, ylim = c(0, 4000))
lines(x = T1_props, y = reef_out2.10, lty = 2)
...
plot(NULL, xlim=c(0,1), ylim=c(0,max(reef_out2.5)), ylab="Number outplanted", xlab="Prop. cement tiles", las = 1)
for(i in 1:length(T1_props)){
  points(x = T1_props[i], y = reef_out2.5[i], pch = 16, col = adjustcolor("dodgerblue2", alpha.f = (reef2.5[i]-min(reef2.5))/(max(reef2.5) - min(reef2.5))), cex = 1.5)
}

plot(NULL, xlim=c(0,1), ylim=c(0,max(reef_out2.10)), ylab="Number outplanted", xlab="Prop. cement tiles", las = 1)
for(i in 1:length(T1_props)){
  points(x = T1_props[i], y = reef_out2.10[i], pch = 16, col = adjustcolor("dodgerblue2", alpha.f = (reef2.10[i]-min(reef2.10))/(max(reef2.10) - min(reef2.10))), cex = 1.5)
}
...
size_props[2, ] <- c(0.6, 0.4, 0, 0, 0) # second lab treatment
lab_pars <- list(s0 = s0, s1 = s1, sett_props = sett_props, size_props = size_props)
...
lab_retain_max_set <- seq(from = 0, to = 1500, length.out = 100)

# holding vectors
# reef population sizes at 5 years
reef1.5 <- rep(NA, length(lab_retain_max_set))
reef2.5 <- rep(NA, length(lab_retain_max_set))

# reef population sizes at 10 years
reef1.10 <- rep(NA, length(lab_retain_max_set))
reef2.10 <- rep(NA, length(lab_retain_max_set))

# intervention reef reproductive output at 5 years
rep1.5 <- rep(NA, length(lab_retain_max_set))
rep2.5 <- rep(NA, length(lab_retain_max_set))

# intervention reef reproductive output at 10 years
rep1.10 <- rep(NA, length(lab_retain_max_set))
rep2.10 <- rep(NA, length(lab_retain_max_set))
...
```{r}
plot(x = lab_retain_max_set, y = reef2.5, type = "l", las = 1)
lines(x = lab_retain_max_set, y = reef1.5, lty = 2)

plot(x = lab_retain_max_set, y = reef2.10, type = "l", las = 1)
lines(x = lab_retain_max_set, y = reef1.10, lty = 2)


# reef2.10
# sum(sim.i$reef_pops_pre[[2]][[1]][,10]) + sum(sim.i$reef_pops_pre[[2]][[2]][,10]) + sum(sim.i$reef_pops_pre[[2]][[3]][,10])
# 
# sum(sim.i$reef_pops_pre[[2]][[1]][,20]) + sum(sim.i$reef_pops_pre[[2]][[2]][,20]) + sum(sim.i$reef_pops_pre[[2]][[3]][,20])
